...
class CirurgiaDAO:
    ...
    def get_cirurgia(self, id_cirurgia):
        ...
        cursor.execute('SELECT * FROM cirurgia WHERE id_cirurgia = %s', (id_cirurgia,))
    ...
    def get_cirurgias(self):
        cursor = db.connection.cursor()
        cursor.execute('SELECT * FROM cirurgia')
    ...
    def get_cirurgias_from_paciente(self, cpf_paciente):
        cursor = db.connection.cursor()
        cursor.execute('SELECT * FROM cirurgia WHERE cpf_paciente = %s', (cpf_paciente,))
        result = cursor.fetchall()
        cursor.close()
